...
// printf-style message
  ...
  #define ASSERT(cond, fmt, ...) \
    do { if (!(cond)) ::assert_min_detail::fail(#cond, __FILE__, __LINE__, __func__, fmt, ##__VA_ARGS__); } while (0)
#else
  // compile away; do NOT evaluate cond or varargs
  #define ASSERT_MSG(cond, fmt, ...) do { (void)sizeof(cond); } while (0)
