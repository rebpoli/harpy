...
AD::Vec VPMatDG::residual_gamma_I_qp( const AD::Vec & /* ad_Uib */ )
{
  ...
  const vector<vector<Real>> & phi_cg_p = fem_p.fe->get_phi();
  const vector<vector<Real>> & phi_cg_n = fem_n.fe->get_phi();
  AD::Vec u_p(3), u_n(3);
  // CG
  for (uint i=0; i<3; i++)
  ...
  // Compute grad_u
  const vector<vector<RealGradient>> & dphi_cg_p = fem_p.fe->get_dphi();
  const vector<vector<RealGradient>> & dphi_cg_n = fem_n.fe->get_dphi();
...
AD::Vec VPMatDG::residual_gammaD_qp( const AD::Vec & /* ad_Uib */ )
{
  ...
  // Compute U , jumps, averages
  const vector<vector<Real>> & phi_cg = fem_p.fe->get_phi();
  const vector<vector<RealGradient>> & dphi_cg = fem_p.fe->get_dphi();
