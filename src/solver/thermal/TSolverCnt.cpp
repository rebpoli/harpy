...
/* */
ThermalSolverConstant::ThermalSolverConstant( ViscoplasticSolver * ref_solver_, string name_ ) :
...
/* */
ThermalSolverConstant::~ThermalSolverConstant() {
  for ( auto & [ sid, mat ] : material_by_sid ) delete( mat );
  material_by_sid.clear();
};

/* */
void ThermalSolverConstant::init()
{
...
/** */
ExplicitThermalMaterial * ThermalSolverConstant::get_material( const Elem & elem )
{ uint sid = elem.subdomain_id(); return get_material( sid ); }
/** */
ExplicitThermalMaterial * ThermalSolverConstant::get_material( uint sid )
{
