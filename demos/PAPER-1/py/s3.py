...
def plot_vertical_profiles(ax, dataset, time_idx, xy_positions, time_series_data, stress_ranges):
    ...
    ax2.set_xlim(-40,0)
    ax2.set_xlabel("Temperature drop (ºC)", fontsize=14)
...
#     ax.set_ylim(1, 0)  # Invert time axis - 1 year only
    ...
    if stress_diffs is not None:
        ...
        text_x = (sxx_min - sxx_max) * .95 + sxx_max
...
def process_timestep_and_save_frame(args):
    ...
    # Create figure
    fig, (ax_profiles, ax_timeseries) = plt.subplots(1, 2, figsize=(12,8), dpi=150)

    # Add figure title
    fig.suptitle(f'Time: {format_time_duration(current_time)}',
                fontsize=14, x=0.01, y=0.999, ha='left', va='top')
    ...
    plt.subplots_adjust(left=0.08, right=0.98, bottom=0.08, top=0.93, hspace=0.08)

    frame_filename = os.path.join(output_dir, f'frame_{frame_idx:04d}.png')
    savefig(fig, frame_filename, quiet=1)
    plt.close(fig)
