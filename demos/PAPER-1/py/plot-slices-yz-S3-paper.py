...
im = ax1.imshow(dummy_data, cmap='RdBu_r', vmin=vmin*1e4, vmax=vmax*1e4)
ax1.clear()
cbar = plt.colorbar(im, cax=ax_cbar)
cbar.set_label(r'Viscoplastic vertical strain, $\varepsilon_{\text{zz}}^{\text{vp}} \text{  }{}_{(\times 10^{4})}$', rotation=270, labelpad=10)
...
ax1.clear()
cbar = plt.colorbar(im, cax=ax_cbar)
cbar.set_label(r'Viscoplastic vertical strain rate, $\dot{\varepsilon}_{zz}^{\text{vp}}$', rotation=270, labelpad=10)
import matplotlib.colors as mcolors
norm = mcolors.TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)
